...
x2.WebFormDesigner = (function() {
    ...
    */
    WebFormDesigner.prototype.updateParams = function (iframeContainer) {
        ...
        /* 
        */
        var embedCode = '<iframe id="up-webform" name="web-form-iframe" src="' + that.iframeSrc + query +
            '" frameborder="0" allowtransparency="true" scrolling="0" width="' + iframeWidth +  '" height="' + 
            iframeHeight + '"></iframe>';

		// Generate The WebForm Id:
        var longquery = query;
		var webformid = longquery.match("FormId=[0-9]?[0-9]?[0-9]?[0-9]?[0-9]");
		
        if ($('#saved-forms').val() != 0) {
            $('#embedcode').val(embedCode);
			$('#webformid').val(webformid);
        } else {
            $('#embedcode').val('');
        }
