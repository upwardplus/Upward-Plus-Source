...
function stageReview(i) {
    ...
    $('#update-button').bind('click.nextStage',function(e){
        ...
        updateHeader.text(<?php echo json_encode($scenario == 'update' ? Yii::t('admin','Updating Upward Plus+...'):Yii::t('admin','Upgrading Upward Plus+...')); ?>);
        proceedToNextStage(ind);
    });
}
...
window.onbeforeunload = function(e) {
    if(isActive())
        return <?php echo json_encode(Yii::t('admin','The updater is currently applying changes to Upward Plus+. If you interrupt it, you could cause SEVERE damage that may include (but would not be limited to) loss of data.')); ?>;
}
...
if(in_array($scenario,array('message','error'))) {
    ...
    if($scenario == 'update'){
        echo Yii::t('admin', 'Update Upward Plus+ to {latestVer} from {currentVer}', array('{latestVer}' => $latestVersion, '{currentVer}' => $version));
    }elseif($scenario == 'upgrade'){
        echo Yii::t('admin', 'Upgrade Upward Plus+');
    } else {
        echo Yii::t('admin','Upward Plus+ Updater');
    }
}
?></h2></div>
...
echo Yii::t('admin','For more information, see {article} in the official Upward Plus+ updating guide.',array('{article}'=>$wikiPage));
?><br /><br />
<?php } ?>

<strong><?php echo Yii::t('admin', 'The following precautions are highly recommended:') ?></strong><br />
<ul style="margin-top:10px;">
    <li><?php echo Yii::t('admin', "Make a backup copy of the database:") ?>
        <ul>
            <li><?php echo Yii::t('admin', 'using third-party web hosting tools, or:'); ?></li>
            <li><?php echo Yii::t('admin', 'by clicking the button below.'); ?></li>
        </ul>
    </li>
    <li><?php echo Yii::t('admin', "Make a backup copy of all the application's files in addition to its database, in case you want to revert to the current version."); ?></li>
    <?php if(file_exists(implode(DIRECTORY_SEPARATOR,array(Yii::app()->basePath,'components','LockAppAction.php')))):
    if(!Yii::app()->locked): ?>
    <li><?php echo CHtml::link(Yii::t('admin','Lock Upward Plus+'),array('/admin/lockApp')); ?></li>
    <?php
    endif;
    endif; ?>
...
<div id="registration-form-container">
        <?php
            echo '<p id="upgrade-step">'.Yii::t('admin','To upgrade, begin by filling out the following form with your registration details. To obtain a license key: see {pp}.',array('{pp}'=>'<a href="http://www.x2engine.com/pricing-plans/" target="_blank">'.Yii::t('admin','pricing plans').'</a>')).'</p>';
