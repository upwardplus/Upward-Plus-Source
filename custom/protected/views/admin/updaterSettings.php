...
<div class='span-24'>
    <div class="form">
        ...
        ?>
        <p><?php echo Yii::t('admin','As often as specified, the application will check for updates and display a system notification message if a new version is available.'); ?></p>
        <hr /><?php
        ...
        $this->widget('CronForm',array(
            ...
            'jobs' => array(
                'app_update' => array(
                    ...
                    'instructions' => Yii::t('admin', 'Specify an update schedule below. Note, Upward Plus+ will be locked when the update is being applied, and so it is recommended to schedule updates at times when the application will encounter the least use. If any compatibility issues are detected, the update package will not be applied, but will be retrieved and unpacked for manual review and confirmation.'),
                )
            ),
        ));
        ...
        <span class="mock-x2-form-label"><?php echo Yii::t('admin','Manual / Offline Update'); ?></span><br />
                <?php
                echo CHtml::tag('p',array(),Yii::t('admin','To update manually, if using the application offline or if something goes wrong, see the instructions given in {wikilink}.',array(
                    '{wikilink}' => CHtml::link(Yii::t('admin','the Update Guide'),'http://wiki.x2engine.com/wiki/Software_Updates_and_Upgrades#Performing_.22Offline.22_Updates')
                )));
