...
// SET GLOBALS //
/////////////////
$document = '<html><header><title>Upward Plus+ System Requirements Check</title>{headerContent}</head><body><div style="width: 680px; border:1px solid #F5F5F5; margin: 25px auto 25px auto; padding: 20px;font-family:sans-serif;">{bodyContent}</div></body></html>';
$totalFailure = array(
	"<h1>This server definitely, most certainly cannot run Upward Plus+.</h1><p>Not even the system requirements checker script itself could run properly on this server. It encountered the following {scenario}:</p>\n<pre style=\"overflow-x:auto;margin:5px;padding:5px;border:1px red dashed;\">\n",
	"\n</pre>"
);
...
if((bool)((int)$maxBytes+1) && $maxBytes <= 33554432) {
    $reqMessages[2][] = installer_t('The memory limit is set to 32 megabytes or lower in the PHP configuration. Please consider raising this limit. Upward Plus+ may otherwise encounter fatal runtime errors.');
}
...
if(!(bool) ($requirements['environment']['fsockopen'] = function_exists('fsockopen'))) {
    $reqMessages[2][] = installer_t('The function "fsockopen" is unavailable or has been disabled on this server. Upward Plus+ will not be able to send email via SMTP.');
}
...
if(!(bool) ($canBackup = $requirements['functions']['proc_open'] = function_exists('proc_open'))) {
    $reqMessages[2][] = installer_t('The function proc_open is unavailable on this system. Upward Plus+ will not be able to control the local cron table, or perform database backups, or automatically restore a database to its backup in the event of a failed update.');
}
...
if(!$canBackup && $requirements['functions']['proc_open']){
	...
	$reqMessages[1][] = installer_t('The "mysqldump" and "mysql" command line utilities are unavailable on this system. Upward Plus+ will not be able to automatically make a backup of its database during software updates, or automatically restore its database in the event of a failed update.');
}
...
if(!($requirements['extensions']['openssl']=extension_loaded('openssl') && $requirements['extensions']['mcrypt']=extension_loaded('mcrypt'))) {
	$reqMessages[1][] = installer_t('The "openssl" and "mcrypt" libraries are not available. If any application credentials (i.e. email account passwords) are entered into Upward Plus+, they  will be stored in the database in plain text (without any encryption whatsoever). Thus, if the database is ever compromised, those passwords will be readable by unauthorized parties.');
}
...
if(!$canInstall){
	$output .= '<div style="width: 100%; text-align:center;"><h1>'.installer_t("Cannot $scenario Upward Plus+")."</h1></div>\n";
	...
	$output .= '<div style="width: 100%; text-align:center;"><h1>'.installer_t('This webserver can run Upward Plus+!').'</h1></div>';
}
