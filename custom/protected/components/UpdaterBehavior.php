...
class UpdaterBehavior extends ResponseBehavior {
    ...
    public function applyFiles($dir=null){
        ...
        if($success)
            ...
            $message = Yii::t('admin', 'Failed to copy one or more files from {dir} into Upward Plus+. You may need to copy them manually.', array('{dir}' => $dir));
    ...
    public function checkIfPackageApplies($throw = true) {
        ...
        if($this->manifest['fromVersion'] != $this->version) {
            if($throw)
                throw new CException(Yii::t('admin','The package to be applied does not correspond to this version of Upward Plus+; it was meant for version {fv} and this installation is at version {av}.',array(
        ...
        if($this->manifest['fromEdition'] != $this->edition) {
            if($throw)
                throw new CException(Yii::t('admin','The package to be applied does not correspond to this edition of Upward Plus+; it was meant for edition "{fe}" and this installation is edition "{ae}".',array(
    ...
    public function renderCompatibilityMessages($h="h3",$htmlOptions=array()) {
        ...
        if($compat['req']['hasMessages']) {
            ...
            if(!empty($definitions)) {
                $header = Yii::t('admin','Some requirements for running Upward Plus+ at the latest version are not met on this server:');
