...
<div class="span-24">
    <div class="form">
        ...
        <p><?php echo Yii::t('admin','Using this form may interfere with third-party cron table managers.')
                .'&nbsp;'.Yii::t('admin','If your hosting service provides a scheduled tasks manager, it is recommended that you use that instead, with the commands as listed here. Contact your account manager.'); ?></p>
        <?php
        ...
        $this->widget('CronForm', array(
            ...
            'jobs' => array(
                'default' => array(
                    'title' => Yii::t('admin', 'Run scheduled tasks via web request'),
                    'longdesc' => Yii::t('admin', 'If enabled, a web request will be made from this web server to itself at the scheduled task runner URL.* This will trigger events such as workflow delayed actions and periodic triggers, and will attempt to send a batch of unsent email campaign messages.'),
                    ...
                    .'<br /><br />'.Yii::t('admin','If the above link does not work, download the script from {link} and copy it to the web root of Upward Plus+. If the script produces a message saying that it cannot resolve the local server, consider disabling this and enabling the alternate scheduled task running method, below.',array('{link}'=>CHtml::link(Yii::t('admin','here'),'https://raw.github.com/X2Engine/X2Engine/master/x2engine/resolve_self.php')) ).'<br /><br />* '.Yii::app()->controller->createAbsoluteUrl('/api/x2cron'),
                ),
                'default_console' => array(
                    'title' => Yii::t('admin','Run scheduled tasks via command line interface'),
                ...
                'app_update' => array(
                    ...
                    'longdesc' => Yii::t('admin', 'If enabled, the application will periodically check for updates and update automatically if a new version is available.'),
                    'instructions' => Yii::t('admin', 'Specify an update schedule below. Note, the application will be locked when the update is being applied, and so it is recommended to schedule updates at times when the application will encounter the least use. If any compatibility issues are detected, the update package will not be applied, but will be retrieved and unpacked for manual review and confirmation.'),
                ),
                'email_logInbound' => array(
                    'title' => Yii::t('admin','Auto-Log Emails'),
                    'longdesc' => Yii::t('admin','If enabled, the application will automatically poll for new inbound and outbound email messages in all of the Email Inboxes with inbound or outbound logging enabled.'),
