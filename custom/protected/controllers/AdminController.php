...
//    }
    ...
    public function actionImportPackage($package) {
        if (Yii::app()->request->isPostRequest) {
            ...
            if (empty($errors)) {
                Yii::app()->user->setFlash('success', Yii::t('admin', 'Finished importing the Package!'));
                Yii::app()->end();
            } else {
...
//      return;
    ...
    public function actionFlowSettings() {
        ...
        if (isset($_POST['Admin']['triggerLogMax'])) {
            ...
            if ($admin->save()) {
                Yii::app()->user->setFlash('success', Yii::t('admin', 'Workflow Settings saved'));
            }
        }
    ...
    public function actionExportMapping() {
        ...
        $name .= " to Upward Plus+ " . Yii::app()->params->version;
...
//  $file = Yii::app()->file->set($this->safePath($file));
    ...
    public function missingClassesException($classes) {
        ...
        $message .= "\n\n" . Yii::t('admin', 'To stop this error from occurring, if the problem persists, restore the file {adminController} to the copy from your version of Upward Plus+:', array('{adminController}' => 'protected/controllers/AdminController.php'));
    ...
    public function checkRemoteMethods() {
        if ($this->noRemoteAccess)
            $this->error500(Yii::t('admin', 'Upward Plus+ needs to retrieve one or more remote files, but no remote access methods are available on this web server, because allow_url_fopen is disabled and the CURL extension is missing.'));
    }
